---
name: soloism
description: Use when two or more concurrent subagents are running through the solo MCP server and one finishing should immediately drive the next, or when spawned subagents go idle unnoticed, linger as stale processes the human must poll or hand-kill, or leave dependent work stalled behind them.
---
...
- The fleet map lives on the board, never in your head: the scratchpad holds pid ↔ task ↔ checkpoint.
...
- **Feedback pane** (cheap model, standing): the human's only typing surface. Its whole job: transcribe each human note to the board as a tagged ticket in the same turn, then wake the steward.

Spawn no extra seats where the lead can hold both duties without work queueing behind it — a small unattended wave. An engaged human still needs a feedback pane however few workers run.
