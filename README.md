# moo

Claude answers fast. moo makes it answer right.

---

Before moo:
> "This should work" → debug → rebuild → debug again

After moo:
> Think → verify → build once

---

## Try it

```bash
/plugin marketplace add saadshahd/moo.md
/plugin install hope@moo.md
```

Then:
```
plan building a markdown-to-PDF CLI
```

Claude will:
- Ask before building
- Search before writing
- Verify before claiming done

---

## Plugins

[hope](docs/plugins/hope.md) · [product](docs/plugins/product.md) · [wordsmith](docs/plugins/wordsmith.md) · [counsel](docs/plugins/counsel.md) · [founder](docs/plugins/founder.md) · [career](docs/plugins/career.md)

→ [Documentation](docs/) · [Issues](https://github.com/saadshahd/moo.md/issues)

---

## Gratitude

[Nate B. Jones](https://www.natebjones.com/) · [Superpowers](https://github.com/obra/superpowers) · [Farnam Street](https://fs.blog/blog/)

---

| Verdict | 🟢 SHIP |
|---------|---------|
| Verified | execution output |
| Checklist | 4/4 |
| Subjective | ~85% |

*Every response ends with evidence, not hope.*
