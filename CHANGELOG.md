...
- feat(hunch): `herd` skill — advisory coordination for concurrent subagents, so delegation runs parallel by default instead of serializing. One ephemeral JSON registry (gitignored `.herd/`, dies with the run) driven by a zero-dep CLI (`herd.mjs`): hierarchical task DAG where owning a node is owning its subtree (any agent decomposes its own slice), TTL resource leases, heartbeat liveness, and a live answer to "who am I, what do I own, what do other running agents own" (`me`/`roster`). Allocation over locking, per Nakajima's *Shared Discovery Paradox* (arXiv:2607.18045) planner-portfolio result: disjoint decomposition prevents conflicts, leases only catch the residue — Monte-Carlo puts locks-only at 0.71× sequential but DAG+leases at 4.49× with waste falling 93.5%→25.4%, robust to non-compliance (3.50× at zero etiquette). Advisory over enforced: conflicts inform with a named owner, never gate (hook philosophy holds); compliance is a measured rate (`events.jsonl`) fenced by a deterministic verify-net (diff vs claim log). Runner-agnostic over harness-coupled: identity arrives via `HERD_ID` env, so native subagents, soloterm, or any shell-capable runner participates — the July 2026 landscape survey found no existing tool covering DAG + leases + liveness for subagents (Beads lacks leases, Agent Mail lacks a DAG and is server-shaped; Beads is the designated exit-ramp if the hunch graduates, per SPEC.md). Validated by a live 3-agent dry-run: engineered shared-file collision caught at claim time, honored without force, zero lost updates, recursion from a child agent. Ships with `HYPOTHESIS.md` (graduation needs ≥10 real sessions: zero lost updates, ≥95% measured compliance, median ≥2× wall-clock, handover probes still landing) and `SPEC.md` (goals, evidence, design stances).
