...
- change(sound): corpus rule files are pure body — the `when: … · tier: … · check: …` metadata line and ALL frontmatter (`paths`, `when`, `source`, `topic`) are gone; a rule file opens with its statement. What was load-bearing moved into the tree: the corpus is laid out `corpus/<tag>/<topic>/<rule>.md` — the top directory is the when-tag (`always`/`react`/`db`/`distributed`; hyphenated `db-distributed` carries both tags), the parent directory is the topic. Facts over fields: directory membership can't drift from a hand-typed frontmatter value, and candidacy/routing become `ls`, not parsing. `paths` (glob-tuning relic) and `tier`/`check` had no consumer anywhere; `source` attribution lives in git history. Consumers updated: `sound:setup` candidacy = tag-directory membership and Write copies files verbatim with topic = parent dir; `install-sound-tree.sh` routes on parent dir; setup-evals `score.mjs` name check recurses. Tier-taxonomy prose mentions reworded to stand alone ("at high-stakes tier" → "in high-stakes code"; "tier-three duplication" → "a duplicated concept"). FIXTURES-SPEC.md drops its stale `globs` section (no fixture carried the key, the scorer never read it).
