...
- change(sound): `sound:setup` collapses to a single delivery home — `.claude/rules/` delivery is dropped entirely. Every selected rule installs as a plain reference file in the browsable `.claude/sound/<topic>/<rule>.md` map, fronted by one minimal `sound-prime` skill (a task-start nudge that makes the agent aware the map exists without enumerating or hardcoding rules). Bodies under `.claude/sound/` never auto-load — the agent pulls one only when it reads it. No more paths-rules, no glob tuning: the paths-tuning phase is deleted and the phases renumbered (Probe → Select → Confirm → Tune examples → Write), and the propose-only JSON no longer emits `globs`. Re-runs reconcile `.claude/sound/` prose-only; the derived `sound-prime` nudge is overwritten verbatim each run.
