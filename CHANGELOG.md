...
- change(sound): the `when: … · tier: … · check: …` metadata line dies — every corpus rule's body now opens directly with its statement. `when:` already lives in frontmatter (the only field selection reads), and no skill or eval ever consumed `tier`/`check`; the two prose mentions of the tier taxonomy are reworded to stand alone ("at high-stakes tier" → "in high-stakes code"; "tier-three duplication" → "a duplicated concept"). `sound:setup` Phase 3 locates the statement as the first prose line after frontmatter, no longer via the metadata line.
- change(sound): dead `paths:` frontmatter removed from every corpus rule — a relic of the deleted glob-tuning era; setup routes on `topic:`, selects on `when:`, and Write strips frontmatter, so nothing consumed it. Rule frontmatter is now `when` / `source` / `topic` only. FIXTURES-SPEC.md drops its stale `globs` section with it (no fixture carried the key, the scorer never read it).
