...
- sound: `sound:setup` collapses to a single delivery home — `.claude/rules/` delivery is dropped entirely. Every selected rule installs as a plain reference file in the browsable `.claude/sound/<topic>/<rule>.md` map, fronted by one minimal `sound-prime` skill (a task-start nudge that makes the agent aware the map exists without enumerating or hardcoding rules). Bodies under `.claude/sound/` never auto-load — the agent pulls one only when it reads it. No more paths-rules, no glob tuning: the paths-tuning phase is deleted and the phases renumbered (Probe → Select → Confirm → Tune examples → Write), and the propose-only JSON no longer emits `globs`. Re-runs reconcile `.claude/sound/` prose-only; the derived `sound-prime` nudge is overwritten verbatim each run.
- sound: `sound:setup` Phase 3 (Confirm) now shows each proposed rule as name + citation **+ its one-line statement** (the rule body's opening mandate — the prose after the `when · tier · check` metadata line, first sentence extended to the mandate sentence for definition-first rules), so approval is informed consent instead of name-only.
...
- sound: removed glob scoring from the install eval — the collapsed single-home delivery has no globs. Dropped the `globs` key from all 9 fixture `expected.json` files and the glob-equivalence machinery (canary-path universe, `matchSet`/`globEq`) from `score.mjs`; the scorer now checks tags + rules only.
