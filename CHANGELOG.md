...
- change(sound): essence trim of `setup` prose (every constraint kept, wording tightened) and `review`; `prime` already minimal. Corpus redundancy pass over all 102 rules: 21 trimmed where an `_Avoid_` clause or example duplicated its `Detect:` verbatim — `Detect:`/`Not-when:` semantics, vocabulary, frontmatter, and Cross-refs untouched on every rule (eval-fence anchors preserved).
