...
- change(repo): the README's "Try it" section becomes "Installation (30-second setup)", cut by install philosophy rather than by layer — a Claude Code branch that subscribes to a managed bundle, and a skills.sh branch that copies editable files onto any agent. The layer cut was the other candidate and lost on honesty: `npx skills add saadshahd/moo.md` reads the whole repo and hands you one flat pick list, so a per-plugin install path would have described a mechanism that does not exist. The non-Claude branch states the cost where it bites — skills land without the plugin prefix, so `hope:intent` becomes `intent` and `sound:review` becomes `review`, and generic names can collide with what a reader already has. The `/hope:router` and `/hope:freeze` bullets are dropped rather than moved: "A mode" and "An anchor" below already carry them. A skills.sh badge goes at the top.
- change(repo): `moo-authoring` declares `metadata.internal: true`. The skills.sh installer walks five directories deep and finds every `SKILL.md` in the tree, so this repo's own authoring doctrine was being offered to installers alongside the shipped skills; the installer honors that flag and filters such a skill out entirely unless `INSTALL_INTERNAL_SKILLS` is set. Claude Code is unaffected — `metadata` is tolerated frontmatter, and the skill is repo-local either way. Both frontmatter rules this cycle produced — that `description` carries the selection decision whole with no `when_to_use`, and that a repo-local skill declares the internal flag — are written into `moo-authoring`'s `## Frontmatter` section, which owns key semantics; the unit-choice table gains only the unit it had no row for, a repo-local skill that never ships. A rule recorded only in a release note is the defect the carve below was written to close.
...
- docs(hope): `slop-nudge.sh`'s header said "persists nothing" while the file wrote three per-transcript markers, the third of which 9.7.6 added and documented eleven lines later as "Keys accumulate" — one file always co-loads with itself, so a reader got both answers. It now names what it writes (chunk offset, stderr log, reported keys) and asserts what is actually true: never work state. The verdict-logic-lives-in-judge.sh clause above the call goes too; `judge.sh` states it with its rationale, and the header already says judge.sh owns the how.
...
- fix(repo): `sync` exits non-zero on a fragment include in a plugin that has no `_fragments/` dir. Such a plugin is filtered out of discovery entirely, so its blocks were never parsed — no transform ran, `errors` stayed empty, and the drift guard passed because nothing changed. The same stale-content-past-the-guard hole the entry below closes, one level up and not reachable from the per-plugin loop that closes it. Found by emptying `hunch/_fragments/` in hunch@0.0.16: `sync --list` fell 16 targets to 7, because dropping a plugin's last fragment silently un-guards **every** skill in that plugin, not just the ones that used it. Verified by injecting an include into a hunch skill — exit 1 naming the file — and by a clean tree still exiting 0.
...
- change(hope): `when_to_use` is folded into `description` across all 20 skills and the field is gone from the repo. It is a Claude Code field, and Claude Code renders the two dash-joined into one listing line — so on Claude Code the merge is byte-identical to what the model already saw, and nothing about selection changes there. Everywhere else it was dead weight: the skills.sh installer copies `SKILL.md` verbatim into `.codex/skills`, `.cursor/skills`, `.github/skills` and the rest, requiring only `name` and `description`, so an agent that does not know the field simply never read the trigger — `hope:intent` arrived as "Turn a rough ask into a clear work order" with the condition that fires it invisible. The merged form is `<what the skill does>. Use when <condition>.` — not the bare dash Claude Code renders. Dropping the key dropped the frame: `when_to_use:` was a typed slot whose name marked the clause as a precondition, and in one flat string a trailing clause reads just as easily as more of what the skill produces. Sixteen conditions took the marker verbatim; four were bare noun phrases (`inversion`, `provocation`, `reframe`, `shift`) and were de-nominalized into finite clauses — grammar, not re-conception. It also ends a split grammar inside one repo, where six of the nine hunch and sound skills already opened `Use when`. This is not the byte-identical change the dash-join alone would have been: it moves the selection surface of all 20 skills at once, taken on the argument with no A/B against the existing `intent` and `shape` evals. Values run 97–172 characters against Claude Code's 1024 cap, and none needed YAML quoting. `compose` is the one description rewritten rather than reframed: it alone was third-person where all 19 siblings are imperative, its head restated its own trigger ("Composes multiple skills" over "the user explicitly names multiple skills"), and "multi aspects/domains" was not grammatical. It now names what the skill does — plan the set, invoke each as its condition arrives — and its condition is the one its own Gate already states.
...
- change(hunch)!: `soloflow` splits the two concepts `contract` was carrying. **contract** now means only the shared boundaries, and the scratchpad section that held them takes the word — it was called `shared`, a second name for the same concept. One of them is a *boundary*, which is what a lane may not change alone. **prompt** takes the other meaning, what a lane is spawned with, so `## Contract` becomes `## Prompt` and Spawn passes `<the lane prompt, substituted>`. Before this, `contract` named the boundaries in Definitions and the lane briefing everywhere else — the `## Contract` heading, Spawn's `<the lane contract, substituted>`, four places in `HYPOTHESIS.md` — while the boundaries themselves went by `shared` in `Record` and throughout the prompt body. Neither half showed from inside the vocabulary section; seeing it took reading Definitions against the rest of the file. The rename carries into `HYPOTHESIS.md`, where the founding bet now reads *lanes that each carry one prompt*; its next clause becomes *the stop-and-ask rule only holds if it arrives at spawn*, since "the prompt holds" names the wrong thing.
...
- feat(hunch): `soloflow` gains **Definitions** and **Mechanics**, and the lead is written as an orchestrator rather than a procedure follower. Definitions fix the five words the skill turns on — you, brief, scope, contract, user — including what the lead lacks: facts, and the user's ubiquitous language. Mechanics hands the lead the cost primitives to plan lanes against rather than a fixed recipe: cost scales as context × turns and not context size, concurrent spawns miss the prompt cache, a worktree carries its own install-and-build cost, `Edit` is a compare-and-swap where `Write` is a blind overwrite, and model tier is spendable. The **Gate** now carries the arithmetic that justifies its own stop — a cold start is tens of thousands of tokens and repo competence costs dollars beyond it — so *one scope, stop* reads as a consequence rather than a rule.
- feat(hunch): `soloflow` extracts the user's mental model before splitting, deferred to `references/mental-model.md` and opened only when it isn't already clear. The reference splits on whether the user has a model yet: draw out an existing one and find the split they would refuse, or build one by putting up decompositions that differ *structurally* rather than cosmetically and letting the reaction be the model forming. This is what `Surface` was already assuming — a human cannot judge progress against a mental model nobody elicited.
...
- feat(hunch): `soloflow` gains **Surface progress**, and this is the release's reason. After spawn the lead brings the environment up and every surface the goal outputs to — repl, storybook, server, playground, a terminal through Solo MCP, a named browser window holding all the needed tabs in one place — so the human watches the work itself rather than an agent's account of it. The lead names what it cannot do and the user must. This is the answer to what the first run exposed: work moved out of the session to be watched directly is not watchable if the only window onto it is a transcript.

### Changed

- change(hunch)!: `soloflow` ends at spawn. **Pause and wait** prints what the user should expect as a deliverable and when visible progress should appear, and the lead stops there. This reverses 0.0.18's deletion of step 4 — *silent at both ends* was the lock, and it is what left the first run's human with nothing to look at. The estimate is a deliberate exception to this repo's no-wall-clock-estimates rule, taken because a hand-off that says only *watch this surface* gives no way to tell a lane that is working from one that is wedged.
- fix(hunch): `soloflow` treated `spawn_agent` as infallible. On the first real run the fourth of four lanes was rejected by the Solo tier limit and the lead absorbed it and worked it inline — readings in `hunch/skills/soloflow/HYPOTHESIS.md`, which until now recorded no run at all. Two lines close it: the lead never works a lane, and a rejected spawn is reported to the user with failure-handling options rather than resolved in the lead's judgment. A pre-spawn capacity gate was built for this and cut before release: capacity really is N−1, since the lead is a process against the same cap, but the cap is not readable from `list_processes`, so the gate could never be passed correctly — the exact shape `HYPOTHESIS.md:31` had already removed once as ritual.
- change(hunch): terminology drift closed — `Soloterm`, `soloterm` and `solo mcp` all become `Solo`, the same drift the soloism review pass closed below.
...
- change(soloflows)!: **the approval surface is gone, and the word "seam" with it.** 0.0.1 had the human classify couplings — *is this a seam?* — and both halves of that failed on contact. The word meant nothing to him, the same objection that killed *"cut"* earlier in this design and that "seam" survived only because the design record kept using it. Worse, the question asked him to judge code relationships from worse information than the agent asking, and ticket 18 had already measured that the split cannot be got right up front by anyone: 42.9% of runs breached, and **an oracle told every file the session would ever touch scored the identical 42.9%**, because the work lands in groups nobody identified at the start. A ruling no one can make correctly is not a gate. `/soloflows:run` now decides the split, spawns, and reports; the human is asked nothing and interrupts if it is wrong.

  This reverses 0.0.1's *print nothing* rule, on that rule's own reasoning. Silence was chosen because every fact a closing message would carry was already known to a human who had just ruled on the split. He no longer rules on anything, so silence would leave him blind to what is running. `shared` replaces `seams` as the record's section name — plain, and self-explanatory to a lane reading it.

  **`PROOF.md` threshold 2b was re-scoped and says so.** It counted turns spent getting a run split and spawned; with no approval surface those turns are now one, so the threshold could not fail. The number 5 is unchanged and what it counts moved to where the cost actually landed — answering lanes that stop, and closing each lane by hand. Flagged in the file itself, because a threshold re-scoped after a design change is exactly the shape of a number being made passable.

  Skill body: 487 → 453 words (1137 at first write).

## [soloflows@0.0.1] - 2026-08-04

### Added

- feat(soloflows): a new plugin — one slash-only skill and `PROOF.md`. `/soloflows:run <goal>` proposes scopes and couplings from disk, asks only about the couplings disk cannot settle, writes one Solo scratchpad as the run's record, spawns each lane in a single call carrying the seams and the contract, and then prints nothing. What it bets on is the contract's **timing**: that a running lane refuses a contract handed to it later, reading it as prompt injection, while an identical lane given it at spawn obeys. That pair is the plugin's whole reason to exist and it is **thinly evidenced** — one lane each way, on `confirm_self_close`, a contract this design has since deleted. The stop-and-ask contract that ships has never been retrofit-tested, and `PROOF.md` names the check that would settle it as worth running before the 5 proof runs. What is better measured is the failure it targets: hand-spawned lanes run past their scope in **43% of runs that could have been split**, silently, surfacing at merge.
...
- change(hunch): the coordination cluster is deleted — `crew`, `roster` and `soloism`, nine skills to six. `_fragments/` empties with them: `board.md` and `steward.md` inlined into crew and soloism and nowhere else, 4,195 bytes that were 54% of crew's own file and were paid twice whenever the two co-loaded, which they did in 5 of the 10 sessions that loaded either. Each skill dies against **its own stated bar**, argued separately; none is superseded by a replacement, and the only thing the three deaths share is hunch's contract that everything graduates or dies. `crew`: the founding thesis resolved against under measurement — 3.8× slower than sequential on the third and largest fixture — because across all three fixtures the environment's implicit coordination (Edit stale-guard, filesystem stigmergy, read-before-write) refused to break even at 10 concurrent agents, so conflict-prevention bought no correctness and its ceremony cost wall-clock; separately, its description named a substrate it never fired on, 32 sessions running `claude -p` with zero genuine auto-selections among them. `roster`: its kill condition named the human *skipping* the approval gate, and prototyping the gate found something worse than skipping — "yes" is one keystroke and "no" costs a full re-proposal, so the cheapest correct-looking move is approval, with every authority present. `soloism`: its own kill condition fired as written, eager dispatch outrunning the human's grasp.

  Deleting now rather than at some later tidy-up is the point of a skill description: descriptions are resident in every session whether or not a body ever loads, so three settled-against bets were being paid for continuously, and a model-invocable one can still be selected into work it now gives wrong guidance for.

### Added

- change(hunch): `hunch/GRADUATED.md` holds the removed skills' `HYPOTHESIS.md` records verbatim, and is created in the same commit that deletes them. The three records are 26,168 of the 27,394 bytes of hypothesis text hunch held across nine skills — the six survivors carry one-line stubs — so deleting the code without keeping the records would discard about 96% of everything this plugin has ever measured, in exchange for nothing. Git archaeology was the alternative and was rejected on that ratio. The file has no frontmatter and is not a `SKILL.md`, so no session loads it, and a table above the records names each skill's bar failure so the reason survives without them. The records themselves are **not** self-contained and are kept anyway: verbatim is the point, so they still cite `../../_fragments/board.md` and sibling `HYPOTHESIS.md` files this same commit deletes, plus a `~/Desktop` grasp dir, a `soloterm.com` docs URL and a memory file outside the repo. A field note that resolved its own paths would be a different document. The header says so rather than leaving a reader to discover it at the first dead link.
...
- change(hope): the slop-nudge Stop hook judges only files version control keeps. A path git ignores, a path that is untracked and unstaged, and a path no worktree contains are each dropped before the judge is spawned — so a charter, a handover note between agents, or a generated one-off never costs a wake. Taste and maintainability are properties of code that lives, or is declared to live, in the repo; nothing else was ever the judge's subject. The rule is deliberately not an extension list: `.md` is this repo's primary artifact surface, so excluding it would silence the judge on skills, `CLAUDE.md`, and this file, and no extension list generalises to a project's own languages anyway. `git add` is the declaration — a brand-new file is admitted the moment it is staged, and the one gap left, a file created and committed without ever being edited after staging, is covered at commit time by `sound:review`. One `git status --porcelain --ignored=matching` per file answers all three questions; if git is absent the filter is skipped rather than dropping everything, since a filter that cannot run must not silently disable the judge. The verdict logic in `judge.sh` is untouched, so the labeled eval set still measures what runs.

  Measured on one real session before shipping: all 7 fires in a requirements-definition session named files under an untracked `.scratch/` workspace, costing ~60 turns and ~34 tool calls, one of them ~22 turns and 9 edits against a map that will be deleted. The filter suppresses all 7. It also suppresses the two that were not hygiene — a stale citation to files a rename had removed, and a missed prior-art check that reopened a locked recommendation — so the trade is known, not assumed. What it does not fix: the judge has the same diminishing tail on tracked files, three fires against `slop-nudge.sh` during this very change, each real and each smaller. Nothing bounds repeat fires against one file.

- docs(hope): `slop-nudge.sh`'s header stops restating what `judge.sh` owns — the `disableAllHooks` recursion guard, the discovered `CLAUDE.md` / `TASTE.md` rubric, the leave-it-better-than-before bar, and the exit-2-wakes / exit-0-silent mapping are each documented at the flag, prompt, or branch that implements them. It also stated the transcript-is-selection rationale a second time, three lines above the block that already carries it at the jq extraction; one file always co-loads with itself, so no compaction boundary licensed the copy. Header 15 lines → 7. Same shape as the `return.sh` / `memory-write.sh` twins removed at 9.7.4, and still unguarded — no shell file in this repo is checked by anything.
...
- feat(sound): `tests/wait-on-a-bound-the-app-emits.md` — before the next input event a retrying assertion is not a wait at all: it resolves at first-true, *earlier* than the sleep it replaced, so the obvious remediation is the regression. That is the rule's whole warrant; no linter or upstream doc carries it. Mined from a Playwright refactor (604 `waitForTimeout` calls to 1) where swapping one sleep for `expect(...).toHaveCount()` turned a passing test red — assertion-only fails, 2-frame settle works, assertion+settle works, the original 300ms sleep works. Bounds are ranked rather than listed: an acknowledgement the app publishes is a causal edge, while a scheduler-step count is an upper bound on a pipeline depth nobody wrote down, admissible only where the test cites the source scheduling it. Where the app emits no readiness signal, that absence is the finding. `Detect:` was rebuilt after scoring 0 of 6 real defects against a 14-case labeled set while flagging correct code twice — it anchored on waits, and the mined case contains none; re-anchored on input pairs and blocking-sleep calls it scores 6 of 6 with no false positives on that set. Key a rule on lexical shapes an agent can find, not judgments it must make correctly. Stated stack-neutrally: the same repo has 9 `Bun.sleep` sites in non-Playwright tests, one waiting on a real `onopen` signal and guessing two lines later. Not written: "don't dissolve a load-bearing wait" (`commit-shape/removal-names-the-fences-reason.md:1`) and the retyped-constant check (`abstraction/duplication-taxonomy-triage.md:2`).
...
- docs(hope): `PHILOSOPHY.md`'s Priming section restated what already co-loads with it — root `PHILOSOPHY.md:12`'s primer parenthetical "(clear intent, shaped approach, engagement level)" verbatim, `:9`'s "check assumptions" verbatim, and a consult-on-non-trivial-decisions clause owned twice over (root `:10` and this file's own **Multiple perspectives**). Co-load verified before acting: root `:31` links here and `CLAUDE.md` names both, so no compaction boundary was available. The one clause with no other owner, "verify before claiming done", duplicates the global `CLAUDE.md`'s **Verify against reality** — nothing survived extraction, so no residue moved to Principles. The section is left carrying only the invocation contract, and is renamed for it.
...
- change(sound): four locked stances re-litigated against dmmulroy's coding-standards skill (crew, one adversarial lane each) — provider-naming KEPT outright (the challenger's own ladder is precedent-gated, so both stances converge greenfield; its additive defaults are exactly the rot case); errors-as-values maximalism REJECTED, but `eliminate-mask-then-surface` gains the foreign-throw ingestion carve — one legal `catch` at the adapter owning a foreign boundary, to turn a dependency's throw into a value, never to steer own flow — and `error-handling-two-regimes`' only-legal-catch sentence is carved to match; `illegal-input-unrepresentable-not-validated` gains the blanket-optional ban (`Partial<T>`/all-fields-optional never a domain input); `comment-must-name-a-consequence` gains a published-surface Not-when (one-line intent + error semantics earn their place on a published module's exported surface; signature-restating boilerplate still doesn't). Micro-folds from the same audit: DTO-never-in-a-symbol-name into `external-contract-not-internal-schema`; state-machine transition guard named as a sanctioned idempotency alternative in `idempotent-handler-by-dedup-key`; `sound:prime` gains a two-sentence conflict note (Not-when/Cross-ref first, correctness outranks style). Review evals not re-run; `review.evals/RESULTS.md` was already flagged STALE.
...
- change(sound): corpus rule files are pure body — the `when: … · tier: … · check: …` metadata line and ALL frontmatter (`paths`, `when`, `source`, `topic`) are gone; a rule file opens with its statement. The corpus is laid out `corpus/<topic>/<rule>.md` — the folder IS the topic, the only fact routing needs. The `when` dimension dies with the frontmatter: candidacy is the ENTIRE corpus, because the tag filter was only ever a pre-filter — the per-rule subject-surface citation is the real gate, and each rule's own `Detect:`/`Not-when:` names the surface it needs. Phase 1 kind-tags remain as verified stack facts anchoring that judgment (still probed, still confirmed, still emitted in propose-only JSON — evals unchanged). `paths` (glob-tuning relic) and `tier`/`check` had no consumer anywhere; `source` attribution lives in git history. Consumers updated: `sound:setup` Write copies files verbatim with topic = parent dir; `install-sound-tree.sh` routes on parent dir; setup-evals `score.mjs` name check recurses. Tier-taxonomy prose mentions reworded to stand alone ("at high-stakes tier" → "in high-stakes code"; "tier-three duplication" → "a duplicated concept"). FIXTURES-SPEC.md drops its stale `globs` section (no fixture carried the key, the scorer never read it).
- change(sound): `sound:setup` drops the example-tuning phase entirely — phases renumbered Probe → Select → Confirm → Write, and Write copies each confirmed rule's file verbatim (no additions). The three tuning gates (syntax-valid TS, de-genericized, grounded) go with it. Re-run reconciliation still compares PROSE ONLY: older installs may carry project-tuned examples and users may add their own snippets — both are user-owned and never regenerated.

### Fixed

- fix(sound): adversarial-review pass over the trim — `concept-count-not-line-count`'s Detect no longer counts conditions that only existed in its deleted example; `invariant-boundary-constructor` field order normalized (Not-when before Cross-ref); two inexact rule references made exact (`command-vs-fact-when-reconciled-elsewhere`, `eliminate-mask-then-surface`'s ladder). Eval fixtures scrubbed of the dead glob-tuning label dimension (all 9 `expected.json` notes + 3 ORIGIN.md purpose claims now describe only tag/rule decisions); `review.evals/RESULTS.md` flagged STALE — its rates of record measured the pre-trim corpus and need a re-run before being trusted.
...
- feat(sound): three corpus rules mined from the same dmmulroy coding-standards audit, where the crew's verdict was GAP (its COVERED verdicts found the corpus sharper on casts and idempotency) — `integration/contain-legacy-pattern-at-the-seam` (new code meets the standard even when neighbors don't: translate the incompatible pattern at the nearest seam, never copy it forward, never over-reach the diff), `naming/parser-prefix-states-input-trust` (`parse*`/`make*`/`assert*` trust contract; a `validate*` returning a refined value is mis-named — it parsed), `abstraction/reuse-audit-before-a-new-holder` (reuse > extend > create; the tell is a second holder for an owned capability; the challenger's ADR gate deliberately dropped — the corpus keys on diff tells, not process steps). A second-pass sweep of the source's remaining veins added two more: `effects/secret-is-wrapped-until-the-point-of-use` (zero prior corpus coverage of secrets-in-observability — a sensitive value is wrapped at its entry boundary, unwrapped only at the single point of use, never in an error field, log, or trace) and `abstraction/depend-on-the-narrow-capability-you-use` (a consumer depends on the smallest capability it actually calls, in its own language — the ISP axis `interface-is-the-full-contract` doesn't cover); plus two folds — a transaction-`await`ing-network-I/O clause into `entity-is-the-atomic-boundary` and an import-time-side-effect clause into `dependency-at-the-edges`. Every other vein verified COVERED, often sharper than the source (module-mock/spy bans, clock/random injection, property testing, barrel files, boolean blindness), or out of corpus scope by design (test-level pyramid, hexagonal layer vocabulary, positive import style).
...
## [hope@9.7.3] - 2026-07-23

### Added

- feat(hope): consult gains two profiles + Domain Map rows — `ousterhout` (software-design-as-craft seat the roster lacked: deep modules, complexity economics, define-errors-out-of-existence; manufactures live dissent with martin on small methods/comments and bloch on minimal surface) and `king` (parse-don't-validate boundary discipline as a single spine; dissents with hickey's "Maybe Not" on optionality-in-types). Cockburn assessed and skipped — `martin` already carries "hexagonal, ports and adapters" verbatim.

### Changed

- change(hope): shape's 3 options must differ on a load-bearing dimension — data ownership, seam placement, coupling, or failure model — never one approach renamed; if only one real path exists, present it alone, not a padded three. Closes the false-trichotomy hole (three cosmetic variants of one decision defeat shape's purpose of surfacing a real HOW choice). Same audit as the hunch fences.

## [hunch@0.0.11] - 2026-07-23

### Changed

- change(hunch): anti-invention fences added where a skill builds a story of a system the agent may not fully know — grasp never invents a change's rationale (says so when it isn't evident), toy never shows invented internals as real (unknown dynamics stay labeled approximate), catch's brevity never drops a caveat, number, warning, or unverified status (bro already had this fence; catch compresses harder and lacked it). One line each, from a crew audit of all 18 skills against dmmulroy's tech-spec skill — the audit's verdict: anti-invention was the single device systematically missing; every other tech-spec device (per-step completion criteria, branch routers, output-shape contracts) is already present in moo's one-gate-per-stage idiom or was rejected as bloat under the 200-line cap.

## [hunch@0.0.10] - 2026-07-23

### Added

- feat(hunch): shared worker-contract fragment `hunch/skills/board.md` — the first doc-gen fragment outside hope — inlined into both crew and soloism SKILL.md (`sync --files` grows by two; each skill names its own bindings for board / claim / peer wake / spawn beside the include). One contract, three parts: awareness (read the board on start, claim mechanically before touching anything, peer-to-peer for everything mechanical, lead = the human's pager), two-tier durable handoff (checkpoint = `$CLAUDE_CODE_SESSION_ID` + one line on what the context holds, written when exploration completes; terminal = cold-successor-complete), and context reuse (kill-by-default — transcripts survive death; successors fork checkpoints via the binding's spawn mechanism with `--resume <id> --fork-session`; checkpoints are immutable so never plain-resume — two resumers collide on one transcript; never `--continue` — latest-in-directory is a race in a shared tree; degradation is always cold start from the terminal handoff). Every mechanism smoke-tested live through solo before writing: session-id self-discovery (env var undocumented, verified 2.1.218; the hook-input `session_id` field is the documented alternative but needs pre-built hook infrastructure, so the contract degrades to no-checkpoint instead), forking a still-running session via `spawn_agent` extra_args with `whoami` identity intact, ~40% first-turn prefix-cache hit on fork (1h-ephemeral tier, full prefix from turn two), and resume-after-kill. Post-draft adversarial instructiveness review (files judged as full units): meta-authoring prose, version stamps, cache-economics justification, a whole-skill recap bullet, and two intra-file duplications cut; two real gaps fixed — the fragment's fork launch is binding-routed (a raw CLI launch would escape solo's tracking) and crew gained the missing spawn binding.

### Changed

- change(hunch): crew + soloism distribute coordination to the workers — first live 0.0.9 session verdict: the lead's turn queue was the bottleneck, holding mechanical coordination the workers could do themselves. Workers now advance what they unblock (claim it, wake the waiting peer, or fork a successor) and contact the lead only for human attention; the lead re-scopes to pager + safety net + reap + integration. Soloism: the done-notify survives as informational fire-and-forget (reap hygiene, never a gate on advance), the fleet map moves from the lead's private map to the scratchpad (written by whoever spawns or reaps), wake gains a reconcile rule (re-make nothing a worker already advanced), and exited-worker redispatch prefers forking the worker's checkpoint over a cold start. Crew: reverses the founding blind-worker stance for awareness only — briefs stay lane-narrow, lane-is-the-claim stands; what's new is workers seeing the lanes. Named risks in both HYPOTHESIS notes: board rot now degrades the human's view (board update is part of *done*), and honor-system board reads escalate to hook-injection if skipped live.

## [hunch@0.0.9] - 2026-07-23

### Changed

- change(hunch): soloism flips from idle-polling to push-notify — every worker brief now ends with a notify contract: write the handoff durably first (scratchpad or todo), then send exactly one `send_input(process_id=<lead>, ...)` naming done|blocked and where the handoff lives; never progress chatter. Verified live before landing: a worker's `send_input` reaches the lead's PTY as a mid-turn message in seconds. The idle watch is demoted to fallback — a plain `timer_set` net (10–20 min) catches workers that die or go silent without notifying, and `timer_fire_when_idle_any` survives only for workers whose environment lacks solo MCP. Notify wakes are narrow (read the sender only, siblings unswept); net fires sweep every live pid. Wakes now scale with completions, not idle flaps (live audit run: 4 of 6 idle wakes were mid-task blips). This supersedes the unreleased cooldown debounce (db16470) — with push primary the cooldown machinery is dead weight, deleted rather than shipped; the root observation stands (solo's idle signal is edge-triggered output-quiescence with no minimum quiet time — a `min_idle_ms` in solo remains the runtime-level fix for the fallback path). Wake triage gains the completed/exited split, mined from the plasma-ai/fractal audit: completed = the handoff satisfies the brief; exited = stopped any other way (context spent, errored, drifted) → capture what stands, redispatch the remainder fresh, never merge as done. Tradeoff named: a silently-crashed worker now waits up to the net delay where the idle watch caught it in one flap.
- change(hunch): crew handoffs get a contract, mined from the fractal audit: a handoff must be a deliverable the worker itself can produce and verify while it runs — never a gate only the lead opens (kills "done when it says done" briefs); and it is written where it survives the worker's session (scratchpad, todo, or file), never terminal output alone (live audit run: 3 of 5 workers' pane-only handoffs scrolled off before capture and needed reprints).

### Removed

- remove(hunch): `.crew-lab/` benchmark workspace deleted — the three-trial battery is concluded and its verdict shipped (crew re-aimed in 0.0.7, CLI retired in 0.0.8). Findings survive where they're load-bearing: crew's HYPOTHESIS.md field notes and the graduation criteria; the fixtures, harnesses, run outputs, and autopsy data were ephemeral evidence, recoverable from git history if a revival condition ever triggers.

## [hunch@0.0.8] - 2026-07-22

### Removed

- remove(hunch): crew CLI retired — `crew.mjs`, `src/`, `SPEC.md` deleted; crew is now a CLI-free discipline skill. The lane assigned at decomposition IS the claim (per-edit claim/release re-negotiated at runtime what the router already decided); three fixtures showed zero conflicts the environment didn't absorb while `--wait` serialized crew's own arm; and after the live-plan posture, an agent-facing registry beside the human-facing tracker was two sources of task truth. SKILL.md rewritten: router records lanes + blocked-by edges in the task tracker the human already observes, verifies each handoff against its declared lane (lane-scoped diff), and embeds worker rules in every brief (stay in lane, batch independent tool calls, sub-lanes traceable through the parent, end with a handoff). Named loss, accepted: proof-grade edit→agent attribution — accountability is now handoffs + lane-scoped diffs. Revival condition recorded in HYPOTHESIS.md (hidden in-flight state whole-rewrite can't absorb, or a live lost update); graduation criterion updated from lease-provable to lane-checkable.

### Changed

- change(hunch): soloism gains a "State + reporting" posture — the human observes through solo, not through the lead's messages: workers monitored via solo only (never make the human poll), each task's todo moves the moment its worker's state moves, decisions land in the scratchpad as they're made, and the lead speaks only when it needs input, an integration decision is ready, or reconciliation changed the plan. The scratchpad and todos are the live plan, not setup artifacts: updated the moment reality diverges (worker discovers context, todo blocks/unblocks, a lane dies, a worker changes files another worker needs, verification fails), and reconciled against what workers learned before any further dispatch — blockers first, dispatch second. The per-completion "report the live children" chatter in the reap step is replaced by the todo/scratchpad routing, and HYPOTHESIS.md's grasp criterion moves with it (human answers "who did what and why" from the live todos/scratchpad, no longer from per-wake reports). Reap step hardened: capture the handoff into scratchpad/todo *before* `close_process` (closing removes the session), keep workers still producing useful work, and inspect a worker's descendants before deciding whether to close the whole group — never close subagents by default. Terminology unified on "worker" across SKILL.md + the graduation criteria (was "child"/"worker" mixed). Batching is now a loop-wide stance (independent calls go in one message; only output-dependence waits a turn) with wave mechanics spelled out: spawn the whole wave in one message, brief it in the next (two turns per wave, never two per worker), and one wake reads every live pid's output in one message (the timer body is delivered verbatim — it never names the idle worker). Stuck workers surface via `select_process` — put the terminal on the human's screen, don't describe it — and are reaped only once the decision no longer needs the live session (was "reap; surface to the human", which destroyed the session before the human could look).

### Fixed

- fix(hunch): review pass verified both skills against the live solo MCP — soloism's wake was armed with a fictional `guard_ms` param (real signature: `timer_fire_when_idle_any(processes, max_wait_ms, body)`), and pids already idle at arm time are silently ignored by `_any`, so the arm response's `already_idle` list now routes straight to the wake step (was a stall-until-deadline gap). Both theses re-anchored post-CLI-retirement: crew's HYPOTHESIS cites the task tracker + handoffs/lane-scoped diffs (was the deleted registry/claim log); soloism's HYPOTHESIS records the collapse to solo todos as the single dependency truth, with a field note marking the founding note's crew-CLI references as historical. Terminology drift closed (residual "agents" → "workers" in crew; "soloterm" → "solo" in soloism).
...
- feat(hunch): `crew` skill — advisory coordination for concurrent subagents, so delegated work runs parallel by default instead of serializing. A fully standalone zero-dep CLI (`crew.mjs`) drives one ephemeral JSON registry: hierarchical task tree where owning a node is owning its subtree (any agent decomposes its own slice), TTL resource leases, heartbeat liveness, and a live answer to "who am I, what do I own, what do other running agents own". The script is the single source of truth — `crew help` documents every command, `crew selftest` proves the tool on any machine (the selftest already earned its keep: it caught a claim-without-start invisibility footgun and a TTL off-by-one before first release) — so neither SKILL.md nor SPEC.md duplicates command semantics. State in the OS temp dir keyed by project path, over a dotdir in the project: nothing pollutes the repo, no gitignore demanded of consumers, ephemerality is physical. Allocation over locking, per Nakajima's *Shared Discovery Paradox* (arXiv:2607.18045) planner-portfolio result: disjoint decomposition prevents conflicts, leases only catch the residue — Monte-Carlo puts leases-only at 0.71× sequential but DAG+leases at 4.49× with waste falling 93.5%→25.4%, robust to non-compliance (3.50× at zero etiquette). Advisory over enforced: conflicts inform with a named owner, never gate (hook philosophy holds); compliance is a measured rate (claim log) fenced by a deterministic verify-net. Runner-agnostic over harness-coupled: identity arrives via `CREW_ID` env, so native subagents, soloterm, or any shell-capable runner participates — the July 2026 landscape survey found no existing tool covering DAG + leases + liveness for subagents (Beads lacks leases, Agent Mail lacks a DAG and is server-shaped; Beads is the designated exit-ramp if the hunch graduates, per SPEC.md). No consumer coupling: existing skills learn about crew only if this hunch graduates. Validated by a live 3-agent dry-run: engineered shared-file collision caught at claim time, honored without force, zero lost updates, recursion from a child agent. Ships with `HYPOTHESIS.md` (graduation needs ≥10 real sessions: zero lost updates, ≥95% measured compliance, median ≥2× wall-clock, the human still owning the story) and `SPEC.md` (goals, evidence, design stances). Organised as single-purpose modules (entry + store/model/commands/selftest); hardened by an eight-agent extraction swarm over heggria/taskflow's edge-case history — corrupt registry fails closed (corrupt ≠ missing), schema stamped, dependency cycles refused, collision-proof temp writes, decomposition warnings past depth 6 / 100 nodes, and child agents are minted their own identity at spawn (env vars inherit — taskflow shipped that exact scar).
...
- feat(hunch): `bro` skill — a one-word manual recovery command (`disable-model-invocation: true`, `effort: low` — a restatement needs speed, not deep reasoning) that restates the last message in plain human language: no jargon, unexplained acronyms spelled out or dropped, short sentences, active voice, substance intact (a caveat, number, or warning never simplifies away). Adopted from [dmmulroy/skills](https://github.com/dmmulroy/skills) — the canonical source, copied verbatim across ~50 dotfiles repos; original X authorship unverified. Corrective counterpart to `plain`: plain shapes prose as it is written, bro recovers when a delivered message still didn't land — the distinction lives here, not in either skill file, so each stays self-contained. Description is the invocation summary, not a trigger — the model never auto-fires it, so the description-trap rule doesn't apply. Ships with `HYPOTHESIS.md`.
