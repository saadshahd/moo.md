...
- change(hunch): soloism debounces flappy idle wakes — solo's idle signal is edge-triggered output-quiescence with no minimum quiet time (live repro: an agent mid-task flaps idle during long tool runs and silent generation, firing the orchestrator's `_any` timer once per flap). After a wake that produced no action, the loop now re-arms through a 30–60s `timer_set` cooldown instead of re-arming the idle watch directly: flapping costs one wake per cooldown, and a genuine completion waits at most the cooldown. Root fix belongs in solo (a `min_idle_ms` on idle timers — the runtime already tracks `idle_seconds`); the cooldown is the skill-level hedge until then.
