...
- feat(hunch): `bro` skill — a one-word manual recovery command (`disable-model-invocation: true`) that restates the last message in plain human language, no jargon, substance intact. Adopted verbatim from a prompt circulating on X; four lines, self-contained. Description is the invocation summary, not a trigger — the model never auto-fires it, so the description-trap rule doesn't apply. Ships with `HYPOTHESIS.md`.
