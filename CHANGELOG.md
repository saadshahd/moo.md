...
- feat(sound): `tests/wait-on-a-bound-the-app-emits.md` — before the next input event a retrying assertion is not a wait at all; it resolves at first-true, which can be *earlier* than the sleep it replaced. So the obvious remediation is the regression, and that is the rule's whole warrant — no external doc or linter carries it. Mined from a refactor that took a Playwright suite from 604 `waitForTimeout` calls to 125 and ~2m to ~0.5m, where swapping one sleep for `expect(...).toHaveCount()` turned a passing test red; bisect: assertion-only fails, 2-frame settle works, assertion+settle works, the original 300ms sleep works. The bounds are ranked rather than listed — an acknowledgement the app publishes is a causal edge, a scheduler-step count is an upper bound on a pipeline depth nobody wrote down, and it is admissible only where app source schedules those steps and the test cites it. Where the app emits no readiness signal, that absence is the finding: the surviving sleeps are a list of places the app cannot say it is done, not compliant residue. `Detect:` defaults to "input" rather than enumerating input methods, since a positive list clears `.hover(`, `.type(`, `goto(`, `dragTo(` and a block-final wait — the exact mined construction — Scored against 14 labeled cases mined from that repo: the first `Detect:` — written as a procedure the agent reasons through — caught 0 of 6 real defects and flagged the correct pattern twice; re-anchored on input pairs and blocking-sleep calls it catches 6 of 6 with no false positives. The lesson is in the rule's shape, not just its text: key on lexical shapes an agent can find, never on judgments it must make correctly, and narrow a clause rather than fencing it. Stated stack-neutrally: the same repo has 9 `Bun.sleep` sites in non-Playwright tests, one waiting on a real `onopen` signal and guessing two lines later. Not written: "don't dissolve a load-bearing wait", already `commit-shape/removal-names-the-fences-reason.md:1`; and the retyped-constant check, owned by `abstraction/duplication-taxonomy-triage.md:2`. The mechanism stays unasserted — the observed data is red/green across four conditions, equally consistent with an undo stack coalescing keypresses, and the probe survives in no ref.
