...
- sound: `topic:` frontmatter on the 45 judgeable taste rules (43 default + 2 opt-in) — the durable, corpus-local source `sound:setup` routes on. A rule with a `topic:` installs into the browsable `.claude/sound/<topic>/` map; one without stays a `.claude/rules/` paths-rule. Folds the former topic-partition design doc into the owned corpus.
...
- sound: `sound:setup` Phase 6 now installs to two homes keyed on a rule's `topic:` frontmatter. Judgeable rules land as plain reference files in a browsable `.claude/sound/<topic>/<rule>.md` map, fronted by a single minimal `sound-prime` skill — a task-start nudge that makes the agent aware the map exists without enumerating or hardcoding rules. Surface-gated / deterministic / process rules stay `.claude/rules/` paths-rules as before. Bodies under `.claude/sound/` sit outside `.claude/rules/`, so they never auto-load — the agent pulls one only when it reads it. Re-runs reconcile both rule homes prose-only; the derived `sound-prime` nudge is overwritten verbatim each run.
