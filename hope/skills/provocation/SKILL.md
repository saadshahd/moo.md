...
Never defend the statement — walking asks what would make it work, never argues that it does. If you catch yourself defending it, it was a proposal all along — the walking ends there.

## Output

The movement is the result — hand it back as the named list: each assumption the obvious answer smuggled in, each option it closed. Discard the statement — it did its work; nothing in the result rests on it or repeats it.

When the walking ended in a defence, hand back the proposal instead — made plainly, as something you stand behind — and claim no movement: nothing was forced into view.

Then check whether anything still unresolved would change what gets built:

- Nothing would → return to the work the provocation interrupted.
- A named assumption is a claim about something that exists and needs a committed verdict → use **judge** skill.
- A closed option can't be chosen until the user has something to react to → use **draft** skill.
- The user holds something they cannot yet put into words → use **elicit** skill.

The provocation is spent once its turn is out — anything further runs under the branch picked here, never as another false statement.
