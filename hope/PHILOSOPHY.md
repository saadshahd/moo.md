...
Pipeline discipline for AI work. Clarify intent, shape approach, execute with verification, consult experts. hope encodes thinking habits as infrastructure so they apply automatically — even when stakes are high and shortcuts tempt.
...
hope primes two things in every conversation:
...
If the user has a matching skill installed, the trigger fires it. If not, hope handles it with its own capabilities. Nothing breaks either way.
...
## Philosophy Audit (Before Committing Changes to hope)
