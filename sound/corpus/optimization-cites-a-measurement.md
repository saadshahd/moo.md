...
RIGHT:
```ts
// profile 2026-06: compile() is 82% of request time, all on repeated inputs (PERF-231)
const parsed = new LruCache<string, Rule>({ max: 500 });
const parseRule = (raw: string): Rule =>
  parsed.getOrInsert(raw, () => compile(raw));
```
...
Cross-ref: speculative-generality-guard — the same defect on the abstraction axis: there the named trigger is a second consumer, here it is a measurement. comment-must-name-a-consequence — the natural home of the citation. steady-state-not-growth-state — any cache the measurement does license still needs a bound.
