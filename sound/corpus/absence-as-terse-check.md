...
WRONG:
```ts
if (user === null || user === undefined) return;      // mechanical enumeration
if (items.length === 0) return;                       // unnamed emptiness
function find(id: string): Option<User> { /* ... */ }  // wrapper over native sugar
```
RIGHT:
```ts
if (!user) return;                     // truthiness is the default question
