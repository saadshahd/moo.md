...
```ts
it('validates order', () => {
  expect(validate(emptyOrder).ok).toBe(false)
  expect(validate(negativeQty).ok).toBe(false)
  expect(validate(validOrder).ok).toBe(true)
})
```
RIGHT:
```ts
it('rejects an order with no line items', () => {
  expect(validate(emptyOrder)).toEqual({ ok: false, error: 'EMPTY_ORDER' })
})
it('rejects a negative quantity', () => {
  expect(validate(negativeQty)).toEqual({ ok: false, error: 'NEGATIVE_QTY' })
})
```
