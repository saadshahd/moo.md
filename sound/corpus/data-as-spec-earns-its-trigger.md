...
WRONG:
```ts
function handle(m: Member) {
  switch (m.tag) {                 // closed, 3 fixed cases — an interpreter buys nothing
    case 'a': return doA(m);
    case 'b': return doB(m);
    case 'c': return doC(m);
  }
...
RIGHT:
```ts
type Member = { tag: 'a' } | { tag: 'b' };
const handlers: Record<Member['tag'], Handler> = { a, b };
// add a tag → the Record fails to compile until you supply its handler
```
