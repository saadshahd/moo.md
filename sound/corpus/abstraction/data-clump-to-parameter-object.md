...
Not-when: a single function with a positional run a caller must order by memory that doesn't recur elsewhere — that is the single-signature grouping handled by object-parameter-over-positional-run, not a recurring data clump.
Cross-ref: object-parameter-over-positional-run — the single-signature grouping rule this extends across signatures.
