...
function render(shape: Shape) {
  switch (shape.kind) {
    ...
    case 'square': return renderSquare(shape);
    // 'triangle' added to Shape later — silently falls through to undefined
  }
}
```
RIGHT:
...
function render(shape: Shape) {
  switch (shape.kind) {
    case 'circle': return renderCircle(shape);
    case 'square': return renderSquare(shape);
    default: return assertNever(shape); // new member -> compile error here, not a prod throw
  }
}
```
