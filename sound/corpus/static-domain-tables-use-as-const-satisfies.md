...
}; // keys ARE checked — but color widens to string; typeof statusConfig.pending.color is string, not 'gray'
```
RIGHT:
```ts
const statusConfig = {
...
// same key/shape check, values keep literal types: 'gray', not string
```
