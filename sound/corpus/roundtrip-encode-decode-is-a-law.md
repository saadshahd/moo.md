...
Not-when: the format is lossy by design (e.g. a display formatter that drops precision) — state the weaker invariant that survives instead (`parse(format(x))` within tolerance), don't force an exact roundtrip on a lossy transform.
