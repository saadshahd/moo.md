...
async function getOrder(id: string) {
  ...
  const data = await res.json() as Order; // asserted, never checked
  return data;
}
```
RIGHT:
```ts
async function getOrder(id: string): Promise<Result<Order, ParseError>> {
  const res = await fetch(`/api/orders/${id}`);
  return Order.parse(await res.json());
  // parse :: unknown => Result<Order, ParseError> — the one boundary where raw becomes typed
}
```
