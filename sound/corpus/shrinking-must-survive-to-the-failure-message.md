...
```ts
try {
  runProperty(genBig, prop, { endOnFailure: true });
} catch {
  console.log('property failed, moving on');
}
```
RIGHT:
```ts
// runProperty is whatever property library the project uses
runProperty(genBig, prop);
// on failure, the runner prints the shrunk counterexample and the seed to reproduce it
```
